...
developer.ajax = {}; // ajax loading
developer.pages = {}; // shared logic for pages
developer.form = {}; // form in page
developer.grid = {}; // grid / lister
...
developer.ajax = {
    ...
    init: function () {
        ...
        window.onpopstate = function (event) {
            ...
            developer.ajax.navigate(document.location, preventPopState);
        };
            ...
            function (event) {
                if (event.target.matches('a[href], a[href] *')) {
                    ...
                    if (url.charAt(0) !== '#' && url.substring(0, 11) !== 'javascript:' && url !== '' && !a.classList.contains('no-ajax') && a.getAttribute('target') !== '_blank') {
                        ...
                        developer.ajax.navigate(url, preventPopState);
                        event.preventDefault();
                    }
                }
            },
        ...
        // now handled by developer.form.initAjax()
    ...
    navigate: function (url, preventPopState) {
        ...
        if (url != document.location.href) {
            ...
            developer.menu.setActivePage(url);
        }
    ...
    setUrl: function (url) {
        if (url != document.location.href && !developer.ajax.currenTarget) {
        ...
        axios(axios_obj)
            .then(function (response) {
                if (typeof result_function === 'function') {
                    ...
                    developer.ajax.done(response);
                }
            })
            .catch(function (error) {
                developer.ajax.error(error);
            })
            .then(function () {
                NProgress.done();
                if (typeof result_function == 'undefined' && !developer.ajax.currenTarget) {
                    developer.pages.init();
                }
            });
    },
    ...
    get: function (url, data, result_function) {
        ...
        let main = false;
        if (developer.ajax.currenTarget) {
            main = developer.ajax.currenTarget;
        }
        if (!main) {
    ...
    error: function (error) {
        if (error.response) {
            ...
            developer.ajax.done(error.response);
...
developer.pages = {
    init: function () {
        this.setTitle();
        developer.menu.setActivePage(window.location.href);
        developer.grid.init();
        developer.grid.inline_edit.init();
        developer.form.init();
        this.initBootstrap();
    },
    ...
    setTitle: function () {
        if (document.querySelector('main h1')) {
            ...
            if (h1_title) {
                document.title = 'Developer | ' + h1_title;
            }
        }
    },
...
developer.collectGarbage = function () {
