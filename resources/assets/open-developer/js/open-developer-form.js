...
developer.form = {
        ...
        Array.from(document.getElementsByTagName('form')).forEach((form) => {
            if (form.getAttribute('pjax-container') != null && !form.classList.contains('has-ajax-handler')) {
                form.addEventListener('submit', function (event) {
                    developer.form.submit(event.target);
        ...
        if (developer.form.validate(form)) {
            if (method === 'post' || method === 'put') {
                ...
                if (typeof result_function !== 'function') {
                    developer.ajax.setUrl(url);
                }
            }

            if (typeof result_function === 'function') {
                developer.ajax.request(url, obj, result_function);
            } else {
                developer.ajax.load(url, obj);
            }
        } else {
    ...
    validate: function (form) {
        ...
        if (form.classList.contains('needs-validation')) {
            ...
            developer.form.check_tab_errors();
        }
        return res;
    },
};
