<div class="dropdown btn-group grid-column-selector dropdown" id="grid-column-selector" data-defaults='{{ implode(",",$defaults) }}'>
    ...
    <ul class="dropdown-menu">
        ...
        <li class="text-right">
            <button class="btn btn-sm btn-light column-select-all" onclick="developer.grid.columns.all()">{{ __('developer.all') }}</button>&nbsp;&nbsp;
            <button class="btn btn-sm btn-primary column-select-submit" onclick="developer.grid.columns.submit()">{{ __('developer.submit') }}</button>
        </li>
    </ul>
</div>
