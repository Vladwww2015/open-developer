...
<footer class="navbar form-footer navbar-light bg-white py-3 px-4 @if (!empty($fixedFooter))shadow fixed-bottom @endif">
    ...
    <div class="col-md-{{$width['field']}} d-flex align-items-center ">
        ...
        <div class="flex-grow-1 ">
            <button type="reset" class="btn btn-warning">{{ trans('developer.reset') }}</button>
        </div>
        @endif
        ...
        @foreach($submit_redirects as $value => $redirect)
            ...
            <div class="form-check form-check-inline">
                ...
                <label class="form-check-label" for="after-save-{{$redirect}}">{{ trans("developer.{$redirect}") }}</label>
            </div>
            @endif
        @endforeach
        </div>
        ...
        <div class="btn-group">
            <button type="submit" class="btn btn-primary">{{ trans('developer.submit') }}</button>
        </div>
