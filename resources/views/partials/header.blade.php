...
<header class="custom-navbar navbar navbar-light bg-white p-0 align-items-stretch">
    <a class="navbar-brand menu-width container-md bg-semi-dark text-center" href="{{ developer_url('/') }}">
        <span class="short">{!! config('developer.logo-mini', config('developer.name')) !!}</span><span class="long">{!! config('developer.logo', config('developer.name')) !!}</span>
    </a>
    <div class="d-flex flex-fill flex-wrap header-items">
        ...
        <ul class="nav navbar-nav hidden-sm visible-lg-block">
            {!! Developer::getNavbar()->render('left') !!}
        </ul>
            ...
            <li class="nav-item">
                <div class="dropdown user-menu d-flex align-items-center px-3" href="#" role="button" id="user-menu-link" data-bs-toggle="dropdown" aria-expanded="false">
                    <span class="bg-light inline rounded-circle user-image">
                        <img src="{{ Developer::user()->avatar }}" alt="User Image">
                    </span>
                    <span class="hidden-xs">{{ Developer::user()->name }}</span>
                </div>
                <ul class="dropdown-menu dropdown-menu-end user-menu" aria-labelledby="user-menu-link">
                    ...
                    <li class="user-header text-center bg-semi-dark p-3">
                        ...
                        <p>
                            <h2>{{ Developer::user()->name }}</h2>
                            <small>Member since developer {{ Developer::user()->created_at }}</small>
                        </p>
                    </li>
                    <li class="user-footer p-2 clearfix">
                        <div class="float-start">
                            <a href="{{ developer_url('auth/setting') }}" class="btn btn-secondary">{{ __('developer.setting') }}</a>
                        </div>
                        <div class="float-end">
                            <a href="{{ developer_url('auth/logout') }}" class="btn no-ajax btn-secondary">{{ __('developer.logout') }}</a>
                        </div>
                    </li>
                </ul>
            </li>
