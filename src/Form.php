...
use OpenDeveloper\Developer\Form\Builder;
use OpenDeveloper\Developer\Form\Concerns\HandleCascadeFields;
use OpenDeveloper\Developer\Form\Concerns\HasFields;
use OpenDeveloper\Developer\Form\Concerns\HasFormAttributes;
use OpenDeveloper\Developer\Form\Concerns\HasHooks;
use OpenDeveloper\Developer\Form\Field;
use OpenDeveloper\Developer\Form\Layout\Layout;
use OpenDeveloper\Developer\Form\Row;
use OpenDeveloper\Developer\Form\Tab;
use OpenDeveloper\Developer\Grid\Tools\BatchEdit;
use OpenDeveloper\Developer\Traits\ShouldSnakeAttributes;
...
class Form implements Renderable
{
    ...
    public function destroy($id)
    {
        try {
            ...
            $response = [
                'status'  => false,
                'message' => $exception->getMessage() ?: trans('developer.delete_failed'),
            ];
        }
    ...
    public function store()
    {
        ...
        if ($response = $this->ajaxResponse(trans('developer.save_succeeded'))) {
            return $response;
        }
    ...
    public function update($id, $data = null)
    {
        ...
        if ($response = $this->ajaxResponse(trans('developer.update_succeeded'))) {
            return $response;
        }
    ...
    protected function redirectAfterSaving($resourcesPath, $key)
    {
        if (request('after-save') == 'continue_editing') {
            ...
            // return message
            return trans('developer.save_succeeded');
            exit;
        } elseif (strpos(request('_previous_'), 'ids')) {
            $url = (new BatchEdit(trans('developer.batch_edit')))->buildBatchUrl($resourcesPath);
        } else {
            $url = request(Builder::PREVIOUS_URL_KEY) ?: $resourcesPath;
        }

        developer_toastr(trans('developer.save_succeeded'));
    ...
    protected function handleColumnUpdates($id, $data)
    {
        ...
        if ($this->handleOrderable($id, $data)) {
            return response([
                'status'  => true,
                'message' => trans('developer.update_succeeded'),
            ]);
        }
     ...
     * @param Closure $callback
     *
     * @return \OpenDeveloper\Developer\Form\Footer
     */
    ...
    public function __call($method, $arguments)
    {
        ...
        developer_error('Error', "Field type [$method] does not exist.");
