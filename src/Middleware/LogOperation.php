...
use OpenDeveloper\Developer\Auth\Database\OperationLog as OperationLogModel;
...
class LogOperation
{
    ...
    {
        $filter = config('developer.operation_log.filter_input', []);
    ...
    {
        return config('developer.operation_log.enable')
    ...
    {
        $allowedMethods = collect(config('developer.operation_log.allowed_methods'))->filter();
    ...
    {
        foreach (config('developer.operation_log.except') as $except) {
            if ($except !== '/') {
                $except = trim($except, '/');
            }
