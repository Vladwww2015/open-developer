...
namespace OpenDeveloper\Developer\Grid\Column;
...
class RangeFilter extends Filter
{
    ...
    {
        if ($this->type == 'time') {
            Developer::script("Inputmask({'mask':'99:99:99'}).mask(document.querySelectorAll('.{$this->class['start']},{$this->class['end']}'));");
        } else {
            $options = [
            ...
            Developer::script("flatpickr('.{$this->class['start']}',{$options});flatpickr('.{$this->class['end']}',{$options});");
        }
    }
