    ...
    protected function addActionScripts()
    {
        $this->actions->each(function ($action) {
            ...
            if (method_exists($action, 'script')) {
                Developer::script($action->script());
            }
        });
    }
    ...
    public function render()
    {
        if (!$this->enableEdit) {
            $this->actions = $this->actions->filter(function ($action, $key) {
                return get_class($action) != "OpenDeveloper\Developer\Grid\Tools\BatchEdit";
            });
        }

        if (!$this->enableDelete) {
            $this->actions = $this->actions->filter(function ($action, $key) {
                return get_class($action) != "OpenDeveloper\Developer\Grid\Tools\BatchDelete";
            });
        }

        $this->addActionScripts();

        return Developer::component('developer::grid.batch-actions', [
