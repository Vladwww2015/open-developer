...
abstract class Extension
{
    ...
    public static function boot()
    {
        ...
        Developer::extend($extension->name, get_called_class());
    ...
    /**
     * Get config set in config/developer.php.
     *
     * @param string $key
     * @param null   $default
     *
    ...
    {
        $name = array_search(get_called_class(), Developer::$extensions);

        if (is_null($key)) {
            $key = sprintf('developer.extensions.%s', strtolower($name));
        } else {
            $key = sprintf('developer.extensions.%s.%s', strtolower($name), $key);
        }
    ...
    /**
     * Import menu item and permission to open-developer.
     */
    ...
    protected function getMenuValidationRules()
    {
        return [
            'title'    => 'required',
            'path'     => ['required', Rule::unique(Config::get('developer.database.menu_table'), 'uri')],
            'icon'     => 'required',
            'children' => 'nullable|array',
        ];
    }
    ...
    protected function getPermissionValidationRules()
    {
        return [
            'name'     => 'required',
            'slug'     => ['required', Rule::unique(Config::get('developer.database.permissions_table'), 'slug')],
            'path'     => 'required|array',
            'path.*'   => 'string',
    ...
    /**
     * Create a item in open-developer left side menu.
     *
     * @param string $title
     * @param string $uri
     * @param string $icon
