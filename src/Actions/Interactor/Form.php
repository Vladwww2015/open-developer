...
namespace OpenDeveloper\Developer\Actions\Interactor;
...
use OpenDeveloper\Developer\Form as ModalForm;
...
class Form extends Interactor
{
    ...
    {
        if ($this->extendsFrom($this->action, 'OpenDeveloper\Developer\Actions\RowAction')) {
    ...
    protected function checkUploadFiel($field)
    {
        ...
        if ($this->extendsFrom($field, 'OpenDeveloper\Developer\Form\Field\File')) {
            $this->multipart = true;
        }
    }
    ...
    protected function resolveView($class)
    {
        ...
        if (!View::exists("developer::form.{$name}")) {
            $name = 'input';
        }

        return "developer::form.{$name}";
    }
    ...
    public function addModalHtml()
    {
        ...
        $modal = view('developer::actions.form.modal', $data)->render();

        Developer::html($modal);
    }
            ...
            document.querySelectorAll('{$this->action->selector($this->action->selectorPrefix)}').forEach(el=>{
                el.addEventListener('{$this->action->event}',function(){
                    ...
                    if (myModalEl.querySelector("[name='_key']").value == ""){
                        myModalEl.querySelector("[name='_key']").value = developer.grid.selected.join();
                    }
