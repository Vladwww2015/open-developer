...
class UserGridTest extends TestCase
{
    ...
    public function testIndexPage()
    {
        ...
        $urlAll = url('/developer/users?_export_=all');
        $urlNew = url('/developer/users/create');
    ...
    public function testGridPagination()
    {
        ...
        $this->visit('developer/users?page=2');
        $this->assertCount(20, $this->crawler()->filter('td a i[class*=fa-edit]'));

        $this->visit('developer/users?page=3');
        $this->assertCount(20, $this->crawler()->filter('td a i[class*=fa-edit]'));

        $this->visit('developer/users?page=4');
        $this->assertCount(5, $this->crawler()->filter('td a i[class*=fa-edit]'));

        $this->click(1)->seePageIs('developer/users?page=1');
        $this->assertCount(20, $this->crawler()->filter('td a i[class*=fa-edit]'));
    }
    ...
    public function testOrderByJson()
    {
        ...
        $this->visit('developer/users?_sort[column]=data.json.field&_sort[type]=desc&_sort[cast]=unsigned');
    ...
    public function testEqualFilter()
    {
        ...
        $this->visit('developer/users?id='.$id)
    ...
    public function testLikeFilter()
    {
        ...
        $this->visit('developer/users?username=mi');
    ...
    public function testFilterRelation()
    {
        ...
        $this->visit('developer/users?email='.$user->email)
            ->seeInElement('td', $user->username)
            ->seeInElement('td', $user->email)
            ->seeInElement('td', $user->mobile)
            ->seeElement("img[src='{$user->avatar}']")
    ...
    public function testGridPerPage()
    {
        ...
        $this->assertEquals('http://localhost:8000/developer/users?per_page=20', $this->crawler()->filter('select option[selected]')->attr('value'));

        $perPage = rand(1, 98);

        $this->visit('developer/users?per_page='.$perPage)
